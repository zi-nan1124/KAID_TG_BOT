...
class MySolanaClient:
    ...
    async def _get_transactions(self, signatures: List[str]):
    ...
    def get_transaction_by_signature(self, signatures: List[str]):
        return asyncio.run(self._get_transactions(signatures))
...
if __name__ == "__main__":
    ...
    signatures = [sample_sig for _ in range(100)]  # 测试 5 个重复签名

    client = MySolanaClient(rpc_url, max_concurrency=50)
    responses = client.get_transaction_by_signature(signatures)
